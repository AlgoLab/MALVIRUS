...
using set_t = unordered_set<vector<string>, vect_str_hash>;
    ...
    for (uint v_index = 0; v_index < variants.size(); ++v_index) {
      ...
      for (const vector<int> &comb : combs) {
        ...
        for (const vector<string> aac : alt_allele_combs) {
          ...
          auto find_allele = _kmers.find(allele_index);
          if (find_allele != _kmers.end()) {
            find_allele->second.push_back(ksss);
          } else {
            _kmers.insert({{ allele_index, vector<vector<string>>({ ksss }) }});
          }
        }
        kmers[v_index] = _kmers;
      }
